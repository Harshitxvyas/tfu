...
def create_funnel_table2(df, platform):
    ...
    # Assign 'Paid' if Comment contains 'Paid' but NOT 'Product'
    mask_paid = (
        ((df_filtered['CampaignName'].str.contains('Paid|conversion|mxpayment', case=False, na=False)) | (df_filtered['AdsetName'].str.contains('mxpayment', case=False, na=False))) &
    ...
    # Assign 'Free' according to platform logic
    if platform.lower() == 'google':
        mask_free = ~df_filtered['CampaignName'].str.contains('Paid', case=False, na=False) &  ~df_filtered['AdName'].str.contains('mxpayment', case=False, na=False)
    elif platform.lower() == 'facebook':
        mask_free = (
        ...
        # Default Free logic if platform is neither Google nor Facebook
        mask_free = (~df_filtered['CampaignName'].str.contains('Paid|conversion', case=False, na=False)
                   &  ~df_filtered['AdsetName'].str.contains('mxpayment', case=False, na=False))
